...
def pred(x, cond, modules, args, device):
    ...
    for i in range(1, args.n_eval):
        ...
        if i < args.n_past:
            ...
            g = modules['frame_predictor'](torch.cat([h, z_t, cond[i-1]], 1)).detach()
            x_in = modules['decoder']([g, skip]).detach()
            #h = modules['encoder'](x_in)[0]#
            gen_seq.append(x_in)
...
def make_gifs(x, cond, modules, epoch, args):
    ...
    for i in range(args.batch_size):
        ...
        fname = '%s/gen/epoch=%d_%d.gif' % (args.log_dir, epoch, i)
