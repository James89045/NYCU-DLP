...
class Discriminator(nn.Module):
    ...
    def forward(self, x, c):
        ...
        out = torch.cat((x,c),dim=1)  # become(B,4,64,64)
        out = self.conv1(out)  # become(B,64,32,32)
        out = self.conv2(out)  # become(B,128,16,16)
        out = self.conv3(out)  # become(B,256,8,8)
        out = self.conv4(out)  # become(B,512,4,4)
        out = self.conv5(out)  # become(B,1,1,1)
...
class Generator(nn.Module):
    ...
    def forward(self, z, c):
        ...
        out = torch.cat((z,c), dim=1)  # (B,z_dim+c_dim,1,1)
        out = self.convT1(out)  # (B,512,4,4)
        out = self.convT2(out)  # (B,256,8,8)
        out = self.convT3(out)  # (B,128,16,16)
        out = self.convT4(out)  # (B,64,32,32)
        out = self.convT5(out)  # (B,3,64,64)
